...
GRANT ALL PRIVILEGES ON DATABASE aegisflow TO aegisflow_user;
...
Create service file `/etc/systemd/system/aegisflow-core.service`:
...
Type=simple
User=www-data
WorkingDirectory=/var/www/aegisflow/modules/CoreAPI
...
sudo systemctl enable aegisflow-core
sudo systemctl start aegisflow-core
```
...
**File: `/etc/nginx/sites-available/aegisflow`**
...
server {
    ...
    # Static files
    location /storage/ {
        alias /var/www/aegisflow/modules/CoreAPI/storage/app/public/;
    }
}
```
